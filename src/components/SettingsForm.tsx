        ...
        <div>
          <DragDropContext
            onDragEnd={({ destination, source }) => {
              form.reorderListItem('settings.playBackModes', {
                from: source.index,
                          ...
                          {(provided) => (
                            ...
                            >
                              <Button.Group key={`${mode}-${index}`}>
                                ...
                                <Button.GroupSection flex={1} variant="default">
          ...
          <Flex justify="flex-end">
            <Menu trigger="click-hover">
              <Menu.Target>
                <Button ml="auto" variant="subtle" rightSection={<IconPlus />}>
                  Add playback mode
                </Button>
              </Menu.Target>
              <Menu.Dropdown>
                {Object.values(PlaybackMode).map((mode) => (
                  <Menu.Item
                    key={mode}
                    onClick={() =>
                      form.insertListItem('settings.playBackModes', mode)
                    }
                  >
                    {playbackModeTranslationMap[mode]}
                  </Menu.Item>
