...
export const scaleSets: QuizSet<Scale>[] = [
  {
    key: 'basic-scales',
    label: 'Basic scales',
    description: { names: basicScales.map(({ name }) => name) },
    options: basicScales,
  },
  {
    key: 'major-modes',
    label: 'Major modes',
    description: { names: majorModes.map(({ name }) => name) },
    options: majorModes,
  },
  {
    key: 'basic-minor',
    label: 'Basic Minor scales',
    description: { names: basicMinorScales.map(({ name }) => name) },
    options: basicMinorScales,
  },
  {
    key: 'all-minor',
    label: 'Advanced Minor scales',
    description: { names: minorScales.map(({ name }) => name) },
    options: minorScales,
  },
  {
    key: 'dominant',
    label: 'Dominant scales',
    description: { names: dominantVariantScales.map(({ name }) => name) },
    options: dominantVariantScales,
  },
  {
    key: 'symmetric',
    label: 'Symmetric scales',
    description: { names: symmetricScales.map(({ name }) => name) },
    options: symmetricScales,
  },
  {
    key: 'basic-mixed',
    label: 'Mixed basic',
    description: { names: basicMixed.map(({ name }) => name) },
    options: basicMixed,
  },
  {
    key: 'intermediate-mixed',
    label: 'Mixed intermediate',
    description: { names: intermadiateMixed.map(({ name }) => name) },
    options: intermadiateMixed,
  },
  {
    key: 'advanced-mixed',
    label: 'Mixed advanced',
    description: { names: advancedMixed.map(({ name }) => name) },
    options: advancedMixed,
  },
  {
    key: 'all',
    label: 'All',
    description: { names: allScales.map(({ name }) => name) },
    options: allScales,
  },
];
